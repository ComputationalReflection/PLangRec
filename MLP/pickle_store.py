...
from parameters import ARG_MAP, INT_LABEL

N_CORES = tf.data.AUTOTUNE


def csv_to_pickles(csv_path: str, instances_per_pickle: int, pickle_name: str, lang_number: int):
    ds = csv_to_int_ds(csv_path)
    ds_labels = ds.map(lambda line: line[-1] if ARG_MAP[INT_LABEL] else line[-lang_number:], num_parallel_calls=N_CORES)
    ds = ds.map(lambda line: line[:-(1 if ARG_MAP[INT_LABEL] else lang_number)], num_parallel_calls=N_CORES)
    ds_to_pickles(ds, ds_labels, instances_per_pickle, pickle_name)
